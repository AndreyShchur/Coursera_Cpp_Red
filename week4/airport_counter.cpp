...
public:
  ...
  AirportCounter(TIterator begin, TIterator end)
  {
      ...
      while (it != end)
      {
          size_t place = static_cast<size_t>(*it);
          airports_[place].second++;
          it++;
      }
  }
  ...
  {
      airports_[static_cast<size_t>(airport)].second++;
  }

  // удалить одно вхождение данного элемента
  void EraseOne(TAirport airport)
  {
      auto place = static_cast<size_t>(airport);
      if (airports_[place].second != 0U)
      {
          airports_[place].second--;
      }
  }
  ...
  {
      airports_[static_cast<size_t>(airport)].second = 0;
  }
...
private:
    ...
    void InitAirportsArray(void)
    {
        int counter = 0;
        for (auto &item : airports_)
        {
            item.first = static_cast<TAirport>(counter);
            item.second = 0;
            counter++;
        }
    }
