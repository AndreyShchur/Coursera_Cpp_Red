...
public:
    ...
    {
        Iterator left_it = first;
        Iterator right_it = next(first, page_size);
        size_t range_size = distance(first, last);

        for (size_t page_number = 0U;
             page_number < (range_size / page_size);
             page_number ++)
        {
            res.push_back(IteratorRange(left_it, right_it));
            left_it = next(left_it, page_size);
            right_it = next(right_it, page_size);
        }

        if (prev(right_it, page_size) != last)
        {
            res.push_back(IteratorRange(left_it, last));
        }
    }
